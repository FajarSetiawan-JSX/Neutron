...
async function get(page = 1) {
    try{
        loading.value = true;
        const response = await axios.get('/api/Admin/alumnis', {
            params:{
                page: page,
                search: search.value,
                ta: ta.value
        ...
        alumnis.value = response?.data?.data;
    ...
    <Auth>
        ...
        <section>
            <div class="p-3 shadow-lg rounded-lg">
                <h1 class="text-lg font-primary">Grafik Alumni</h1>
                <Line :data="data" :options="options" class="max-h-72" />
            </div>
        </section>
        <section class="my-5">
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">TA Lulus</th>
                                    ...
                                    <tr>
                                        <td colspan="6">
                                            <div class="my-5">
                                                ...
                                                <h1 class="my-5 font-anonymous text-red-500 text-center">Fetching data alumni</h1>
                                            </div>
                                        </td>
                                    </tr>
                                </template>
                                <template v-else>
                                    <template v-if="alumnis.length > 0">
                                        <tr v-for="(siswa, index) in alumnis" :key="siswa?.id" class="border-b">
                                            ...
                                            <td class="px-4 py-3">{{ siswa?.ta }}</td>
                                    ...
                                    <template v-else>
                                        <tr>
                                            <td colspan="6">
                                                <h1 class="my-5 font-anonymous text-red-500 text-center">Tidak ada alumni</h1>
                                            </td>
                                        </tr>
                                    </template>
                                </template>
        ...
        <PaginationCard :links="links" :name="'Alumni'" @next="handlenextpage" @page="handlepage" @prev="handleprevpage" />
    </Auth>
</template>