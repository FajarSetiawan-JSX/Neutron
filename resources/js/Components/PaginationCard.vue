...
<template>
    <div class="flex items-center justify-between border border-zinc-200 bg-white px-6 py-4 shadow-sm">
        ...
        <div class="flex items-center gap-2">
            <button type="button" :disabled="props?.links?.current_page === 1" @click="emit('prev', props?.links?.current_page - 1)"
                :class="
            ...
            <template v-for="(page, index) in pages" :key="index">
                <span v-if="page === '...'" class="flex h-10 w-10 items-center justify-center text-sm text-zinc-400">
                    ...
                </span>
                <button v-else type="button" @click="emit('page', page)" class="flex h-10 w-10 items-center justify-center rounded-lg border text-sm font-semibold transition"
                    :class="
            ...
            <button type="button"
                :disabled="
                    props?.links?.last_page == props?.links?.current_page
                "
                @click="
                    emit('next', props?.links?.current_page + 1)
                "
                :class="
                    props?.links?.last_page == props?.links?.current_page
                        ? 'opacity-50'
