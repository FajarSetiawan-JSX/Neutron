...
import { CircleArrowOutUpRight, TriangleAlert } from "lucide-vue-next";
...
async function hapus() {
    try{
        loading.value = true;
        const response = await axios.delete(`/api/Admin/siswa/${props?.siswa?.id}/rombel/${props?.rombel?.id}`);
        success('Berhasil menghapus rombel 🥲');
        //success('Lagi maintenance 😁');
        emit('success')
    }catch(error){
            ...
            <CardBorderAnimation @click.stop data-aos="zoom-in">
                <form @submit.prevent="hapus">
                    <div class="rounded-xl bg-black p-6 w-md">
                        ...
                        <div class="my-4">
                            <h1 class="text-white font-primary text-xl font-semibold text-center mb-2">Hapus rombel {{ props?.rombel?.nama }}?</h1>
                            <p class="text-center text-white/70 font-primary text-sm font-semibold">Yakin nih <span class="text-white">{{ props?.siswa?.nama }}</span> mau hapus rombel <span class="text-white">{{ props?.rombel?.nama }}</span> ?  nanti ga bisa ketemu sama Tentor <span class="text-white">{{ props?.rombel?.tentor }}</span> lagi <span class="text-white">🥲</span></p>
                        </div>
                        <div class="mt-10 flex justify-end gap-3">
                            <GradientButton color="red" type="submit" :disabled="loading" :class="loading ? 'opacity-50 cursor-not-allowed' : 'opacity-100 cursor-pointer'" class="flex items-center justify-center w-full gap-2 font-primary py-3 px-6 text-white rounded-xl">
                                <CircleArrowOutUpRight v-if="!loading" class="w-5 h-5"/>
                                <PrimaryLoading v-else :size="20" :class="'stroke-white'" />
                                {{ loading ? '' : 'Keluar rombel' }}
                            </GradientButton>
                        </div>
                    </div>
                </form>
