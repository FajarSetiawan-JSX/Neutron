...
async function hapus() {
    try{
        loading.value = true;
        const response = await axios.delete(`/api/Admin/mapel/${id.value}`);
            ...
            <CardBorderAnimation @click.stop data-aos="zoom-in">
                <form @submit.prevent="hapus">
                    <div class="rounded-xl bg-black p-6 w-md">
                        ...
                        <div class="my-4">
                            <h1 class="text-white font-primary text-xl font-semibold text-center mb-2">Hapus Mata Pelajaran?</h1>
                            <p class="text-center text-white/70 font-primary text-sm font-semibold">Menghapus mapel berarti tentor tidak lagi mengajar dan siswa tidak lagi mengambil mapel ini.</p>
                        </div>
