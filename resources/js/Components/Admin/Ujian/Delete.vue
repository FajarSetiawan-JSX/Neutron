...
async function hapus() {
    try{
        loading.value = true
        const response = await axios.delete(`/api/Admin/ujian/${id.value}`);
        success('Berhasil menghapus tipe ujian');
        emit('success');
    }catch(error){
            ...
            <CardBorderAnimation @click.stop data-aos="zoom-in">
                <form @submit.prevent="hapus">
                    <div class="rounded-xl bg-black p-6 w-md">
                        ...
                        <div class="my-4">
                            <h1 class="text-white font-primary text-xl font-semibold text-center mb-2">Hapus Tipe Ujian?</h1>
                            <p class="text-center text-white/70 font-primary text-sm font-semibold">Anda mungkin tidak diizinkan oleh sistem untuk menghapus tipe ujian untuk alasan integritas data.</p>
                        </div>
