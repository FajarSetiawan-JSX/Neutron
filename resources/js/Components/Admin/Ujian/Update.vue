...
async function update() {
    try{
        loading.value = true;
        const response = await axios.patch(`/api/Admin/ujian/${form.value.id}`, form.value);
        success('Berhasil memperbarui tipe ujian 🫡');
        emit('success')
    }catch(error){
...
watch(()=>props.ujian, (newujian)=>{
    if(newujian){
        err.value = {};
        form.value.id = newujian.id;
        form.value.nama = newujian.nama;
        form.value.deskripsi = newujian.deskripsi;
    }
},{immediate:true})
</script>
