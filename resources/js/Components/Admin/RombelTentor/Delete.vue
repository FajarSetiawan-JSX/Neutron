...
async function hapus() {
    try{
        loading.value = true;
        // const response = await axios.delete(`/api/Admin/rombel/${props?.rombel?.id}`);
                ...
                <form @submit.prevent="hapus">
                    <div class="rounded-xl bg-black p-6 w-md">
                        ...
                        <div class="my-4">
                            ...
                            <p class="text-center text-white/70 font-primary text-sm font-semibold">Yakin nih mau hapus rombel <span class="text-white">{{ props?.rombel?.nama }}</span> tahun ajaran <span class="text-white">{{ props?.rombel?.ta }}</span> ? kalo rombel ini dihapus maka siswa tidak lagi ngambil rombel ini, termasuk data absensi dan nilai juga ilang loh! 😡</p>
                        </div>
